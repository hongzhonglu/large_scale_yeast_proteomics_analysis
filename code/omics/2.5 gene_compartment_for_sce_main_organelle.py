...
# function to refine the manual curation from SGD database
...
### output ###
df_renamed = gene_sgd_2026[gene_sgd_2026['gene'].isin(compartment1['mitochondrion_sgd_2026_reduce_glycolysis_refine'])]   # update the name
df_renamed.to_excel("data/sce_compartment_curation/2026_curated/mitochondrion_annotations.xlsx") # update the name
len(set(df_renamed['gene'].tolist()))
...
### output ###
df_update = gene_sgd_2026[gene_sgd_2026['gene'].isin(compartment1['plasma_sgd_2026_refine_reduce_glycolysis'])]   # update the name
df_renamed = df_update[df_update['gene'].isin(membrane_gene_list)]
...
### output ###
df_renamed = pd.DataFrame({"gene": compartment1['cytosol']})
df_renamed.to_excel("data/sce_compartment_curation/2026_curated/cytosol_annotations.xlsx")
len(set(df_renamed['gene'].tolist()))
...
### output ###
df_renamed = gene_sgd_2026[gene_sgd_2026['gene'].isin(compartment1['lipid_droplet'])]   # update the name
...
# other batch process with no further new curation
# fungaltype_vacuole
# 这个未用到
...
### output ###
df_renamed = gene_sgd_2026[gene_sgd_2026['gene'].isin(compartment1['peroxisome'])]   # update the name
...
### output ###
df_renamed = gene_sgd_2026[gene_sgd_2026['gene'].isin(compartment1['nucleolus'])]   # update the name
df_renamed.to_excel("data/sce_compartment_curation/2026_curated/nucleolus_annotations.xlsx") # update the name
len(set(df_renamed['gene'].tolist()))
# based on the curation, nucleolus only has 217 proteins
...
# three emp protein
check_list = ['YJL052W','YJR009C','YGR192C']
fungaltype_cell_wall = fungaltype_cell_wall[~fungaltype_cell_wall['gene'].isin(check_list)]
fungaltype_cell_wall.to_excel('data/sce_compartment_curation/2026_curated/fungal_type_cell_wall_annotations.xlsx')
len(set(fungaltype_cell_wall['gene'].tolist()))
...
len(set(endoplasmic_reticulum_membrane['gene'].tolist()))
...
len(set(Golgi_membrane['gene'].tolist()))
...
compartment20 = compartment2[compartment2['GO_Name'] =='Golgi membrane']
compartment20_check = compartment20[~compartment20['Systematic_name'].isin(Golgi_membrane['gene'])]